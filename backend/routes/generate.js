...
// @access  Private
router.post('/text-to-image', auth, async (req, res) => {
  console.log('🔍 [BACKEND DEBUG] Text-to-image route called');
  
  try {
    const { prompt, preset, size = '1024x1024', quality = 'standard', style = 'vivid' } = req.body;
    
    console.log('🔍 [BACKEND DEBUG] Request body:', {
    ...
    const generationsLimit = quota.generationsLimit;

    if (generationsUsed >= generationsLimit) {
      console.error('❌ [BACKEND DEBUG] Quota exceeded:', { generationsUsed, generationsLimit });
    ...
    console.log('✅ [BACKEND DEBUG] Quota check passed:', { generationsUsed, generationsLimit });

    // Process preset if provided
    let finalPrompt = prompt;
    if (preset) {
      console.log('🔍 [BACKEND DEBUG] Processing preset:', preset);
      
      try {
        // Try to get preset from CSV first
        const allPrompts = await promptMatcher.getAllPrompts();
        let presetData = allPrompts.find(p => p.id === preset || p.name === preset);
        
        // Fallback to hardcoded presets if not found in CSV
        if (!presetData) {
          presetData = FALLBACK_PRESETS[preset] || Object.values(FALLBACK_PRESETS).find(p => p.id === preset || p.name === preset);
        }
        
        if (presetData) {
          // Replace {prompt} placeholder or append to main prompt
          if (presetData.main_prompt.includes('{prompt}')) {
            finalPrompt = presetData.main_prompt.replace('{prompt}', prompt);
          } else {
            finalPrompt = `${presetData.main_prompt}, ${prompt}`;
          }
          
          console.log('✅ [BACKEND DEBUG] Preset applied:', {
          ...
          console.warn('⚠️ [BACKEND DEBUG] Preset not found, using original prompt');
        }
      } catch (error) {
        console.error('❌ [BACKEND DEBUG] Error processing preset:', error);
        console.warn('⚠️ [BACKEND DEBUG] Using original prompt due to preset error');
      }
    }

    // Explicitly log the full final prompt that will be sent to AI
    console.log('🧪 [BACKEND DEBUG] Final prompt to AI (text-to-image):', finalPrompt);

    // Call Gemini service
    console.log('🔍 [BACKEND DEBUG] Calling Gemini service...');
    
    let imageUrl;
    if (!geminiService) {
      console.log('⚠️ [BACKEND DEBUG] Gemini service not available, generating local placeholder image');
      try {
        ...
        console.log('✅ [BACKEND DEBUG] Local placeholder image generated:', imageUrl);
      } catch (genErr) {
        console.error('❌ [BACKEND DEBUG] Failed to generate local placeholder image:', genErr);
        imageUrl = `${req.protocol}://${req.get('host')}/uploads/placeholder.jpg`;
      }
    } else {
      try {
        const imageResult = await geminiImageService.generateImage(finalPrompt, {
        ...
        console.log('✅ [BACKEND DEBUG] Gemini service response received');
        
        if (!imageResult || !imageResult.imageUrl) {
          console.warn('⚠️ [BACKEND DEBUG] Service returned fallback response, generating local placeholder image:', imageResult);
          try {
            const uploadsDir = path.join(__dirname, '../uploads');
            if (!fs.existsSync(uploadsDir)) fs.mkdirSync(uploadsDir, { recursive: true });
            const [w, h] = (size || '1024x1024').split('x').map(n => parseInt(n, 10));
        ...
        // Fallback to locally generated placeholder if external placeholder detected
        if (/^https?:\/\/via\.placeholder\.com/i.test(imageUrl)) {
          console.log('⚠️ [BACKEND DEBUG] Placeholder image URL detected. Generating local placeholder file...');
          try {
            const uploadsDir = path.join(__dirname, '../uploads');
            if (!fs.existsSync(uploadsDir)) fs.mkdirSync(uploadsDir, { recursive: true });
            const [w, h] = (size || '1024x1024').split('x').map(n => parseInt(n, 10));
            ...
            console.log('✅ [BACKEND DEBUG] Using local placeholder image instead:', localUrl);
            imageUrl = localUrl;
          } catch (genErr) {
            console.error('❌ [BACKEND DEBUG] Failed to generate local placeholder image:', genErr);
          }
        }
        console.log('✅ [BACKEND DEBUG] Generated image URL:', imageUrl);
      } catch (error) {
        console.error('❌ [BACKEND DEBUG] Unexpected service error, using fallback:', error);
        // Fallback to local placeholder generation if service throws unexpected errors
        try {
          const uploadsDir = path.join(__dirname, '../uploads');
          if (!fs.existsSync(uploadsDir)) fs.mkdirSync(uploadsDir, { recursive: true });
          const [w, h] = (size || '1024x1024').split('x').map(n => parseInt(n, 10));
          ...
          console.error('❌ [BACKEND DEBUG] Failed to generate fallback image:', fallbackErr);
          imageUrl = `${req.protocol}://${req.get('host')}/uploads/placeholder.jpg`;
        }
      }
    }
    ...
    console.log('✅ [BACKEND DEBUG] User quota updated');

    // Create generation record
    const generation = new Generation({
      userId: req.user.id,
      // For text-to-image, there is no original input image
    ...
    console.error('❌ [BACKEND DEBUG] Text-to-image generation error:', {
...
router.post('/retry/:id', auth, asyncHandler(async (req, res) => {
  ...
  // Redirect to text-to-image generation logic
